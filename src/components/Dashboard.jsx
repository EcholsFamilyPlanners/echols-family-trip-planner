...
export default function Dashboard({ destinations, statusOf, favoriteOf, openTrip, toggleFavorite, venues, packingItems, ideaInbox, setIdeaInbox }) {
  ...
  return <>
    ...
    <section className="panel"><h2>Top Picks</h2><div className="miniGrid">{top.slice(0,6).map(t=><TripCard key={t.id} trip={t} status={statusOf(t)} favorite={favoriteOf(t)} openTrip={openTrip} toggleFavorite={toggleFavorite}/>)}</div></section>
  </>
}

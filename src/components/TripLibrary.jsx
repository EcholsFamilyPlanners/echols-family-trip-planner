...
export default function TripLibrary({ destinations, statusOf, favoriteOf, voteOf, coverPhotos, destPhotos, openTrip, toggleFavorite }) {
  ...
  return <>
    ...
    <section className="grid">{filtered.map(t=><TripCard key={t.id} trip={t} status={statusOf(t)} favorite={favoriteOf(t)} vote={voteOf ? voteOf(t) : null} coverPhoto={coverPhotos?.[t.id]} destPhoto={destPhotos?.[t.id]} openTrip={openTrip} toggleFavorite={toggleFavorite}/>)}</section>
  </>
}

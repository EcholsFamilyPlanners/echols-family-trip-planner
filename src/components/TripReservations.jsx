...
const BLANK = { name:'', res_type:'Flight', confirmation:'', res_date:'', cost:'', status:'Pending', url:'', notes:'' };
...
export default function TripReservations({ tripId }) {
  ...
  const save = async () => {
    ...
    await saveReservation({ ...form, trip_id: tripId, cost: form.cost ? Number(form.cost) : null, res_date: form.res_date || null });
    setForm(null);
    await load();
  };
          ...
          <div key={r.id} className={`resCard ${STATUS_COLORS[r.status]||''}`}>
            <div className="resCardLeft">
              ...
              <div className="resCardInfo">
                ...
                <div className="resMeta">
                  {r.res_date && <span>📅 {new Date(r.res_date + 'T12:00:00').toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}</span>}
            ...
            <div className="resCardRight">
              ...
              <div className="resActions">
                ...
                <button className="btn secondary small" onClick={()=>setForm({...r, res_date: r.res_date||'', cost: r.cost??'', confirmation: r.confirmation||'', url: r.url||'', notes: r.notes||''})}>Edit</button>

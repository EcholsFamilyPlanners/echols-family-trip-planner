...
export default function CouplesPlanner({
  ...
  allVotes, myVotes, coverPhotos, destPhotos, togetherNotes, updateTogetherNotes,
        ...
        <section className="panel bothLovePanel">
          ...
          <div className="grid">
            {bothLove.map(({trip, score}) => (
              <TripCard key={trip.id} trip={trip} status={statusOf(trip)} favorite={favoriteOf(trip)} vote={myVotes?.[trip.id] || null} coverPhoto={coverPhotos?.[trip.id]} destPhoto={destPhotos?.[trip.id]} openTrip={openTrip} toggleFavorite={toggleFavorite}/>
            ))}
          </div>
        </section>
      )}

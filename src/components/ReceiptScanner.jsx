...
export default function ReceiptScanner({ tripId, onSaved }) {
  ...
  const handleFile = async (file) => {
    ...
    try {
      // Step 1: upload the raw photo to Supabase Storage (same bucket as trip photos)
      setProgress('Uploading receipt...');
      const ext = file.name.split('.').pop() || 'jpg';
      const path = `receipts/00000000-0000-0000-0000-000000000001/${tripId}/${Date.now()}.${ext}`;
      ...
      // Step 2: kick off the job with just the small image URL, not the bytes
      setProgress('Reading receipt with AI...');
      const startRes = await fetch('/.netlify/functions/start-receipt-scan', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tripId, imageUrl: urlData.publicUrl }),
      });

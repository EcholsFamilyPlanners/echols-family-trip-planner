...
function App(){
  ...
  const [packingTemplates,setPackingTemplates]=useState([]); const [packingItems,setPackingItems]=useState([]); const [sportsVenues,setSportsVenues]=useState([]); const [householdMembers,setHouseholdMembers]=useState([]); const [allPersonalTripData,setAllPersonalTripData]=useState([]);
  ...
  const refresh=async()=>{const data=await loadAllData(seedDestinations);setSharedTripData(data.sharedTripData);setPersonalTripData(data.personalTripData);setAllPersonalTripData(data.allPersonalTripData || []);setHouseholdMembers(data.householdMembers || []);setCustomTrips(data.customTrips);setPackingTemplates(data.packingTemplates);setPackingItems(data.packingItems);setSportsVenues(data.sportsVenues);}
  useEffect(()=>{getSession().then(async s=>{setSession(s);await ensureHouseholdMember(s);await refresh();});return onAuthChange(async s=>{setSession(s);await ensureHouseholdMember(s);await refresh();})},[]);
